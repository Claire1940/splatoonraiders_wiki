...
export async function GET() {
  const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.splatoonraiders.wiki'
  const siteName = process.env.NEXT_PUBLIC_SITE_NAME || 'Splatoon Raiders Wiki'

  const lines: string[] = []
  lines.push(`# ${siteName}`)
  lines.push('')
  lines.push(
    `> ${siteName} is a fan information hub focused on release updates, trailers, gameplay systems, and official source links for Splatoon Raiders.`
  )
  lines.push('')
